...
This project explores the application of Reinforcement Learning (RL) to develop defensive agents capable of countering cyberattacks. Two RL algorithms were implemented: **SARSA**, an on-policy method using a Q-table, and **Double Deep Q-Network (DDQN)**, an off-policy deep learning-based approach. These agents were trained and evaluated using the [gym-idsgame](https://github.com/ebagdasa/gym-idsgame) environment under two attack scenarios: **random attack** and **maximal attack**.
...
- [SARSA Implementation](#sarsa-implementation)
  ...
  - [Analysis](#analysis)
- [DDQN Implementation](#ddqn-implementation)
...
**SARSA** (State-Action-Reward-State-Action) is an on-policy tabular RL algorithm. It updates its Q-values based on the actions it actually takes, allowing the agent to continuously adapt its behavior as it interacts with the environment.

SARSA is well-suited for relatively simple, discrete environments—like random attack simulations—where a compact state-action space allows for efficient Q-table management.
...
- **Fluctuations**: The on-policy nature of SARSA causes reward variability due to its ongoing exploration.
